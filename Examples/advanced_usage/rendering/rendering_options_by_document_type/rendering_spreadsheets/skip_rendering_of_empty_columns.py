# skip_rendering_of_empty_columns.py
...
def run():
    ...
    with gv.Viewer(test_files.sample_xlsx_with_empty_column) as viewer:
        options = gvo.HtmlViewOptions.for_embedded_resources(page_file_path_format)
        options.spreadsheet_options.skip_empty_columns = True
        viewer.view(options)
