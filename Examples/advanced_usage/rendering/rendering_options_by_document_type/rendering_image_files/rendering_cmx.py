...
def run():
    ...
    page_file_path_format = join(output_directory, "cmx_result_{0}.html")
    pages = [1]
    # TO HTML
    with gv.Viewer(test_files.sample_cmx) as viewer:
        options = gvo.HtmlViewOptions.for_embedded_resources(page_file_path_format)
        viewer.view(options, pages)  # Render the first image

    # TO JPG
    page_file_path_format = join(output_directory, "cmx_result_{0}.jpg")
    with gv.Viewer(test_files.sample_cmx) as viewer:
        options = gvo.JpgViewOptions(page_file_path_format)
        viewer.view(options, pages)  # Render the first image

    # TO PNG
    page_file_path_format = join(output_directory, "cmx_result_{0}.png")
    with gv.Viewer(test_files.sample_cmx) as viewer:
        options = gvo.PngViewOptions(page_file_path_format)
        viewer.view(options, pages)  # Render the first image

    # TO PDF
    page_file_path_format = join(output_directory, "cmx_result.pdf")
    with gv.Viewer(test_files.sample_cmx) as viewer:
