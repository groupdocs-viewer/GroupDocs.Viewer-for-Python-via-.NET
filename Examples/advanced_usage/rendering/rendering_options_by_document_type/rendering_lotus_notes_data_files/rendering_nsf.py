...
def run():
    ...
    page_file_path_format = join(output_directory, "NSF_result.html")
    ...
    # TO JPG
    page_file_path_format = join(output_directory, "NSF_result_{0}.jpg")
    with gv.Viewer(test_files.sample_nsf) as viewer:
        options = gvo.JpgViewOptions(page_file_path_format)
        viewer.view(options)
    
    # TO PNG
    page_file_path_format = join(output_directory, "NSF_result_{0}.png")
    with gv.Viewer(test_files.sample_nsf) as viewer:
        options = gvo.PngViewOptions(page_file_path_format)
        viewer.view(options)
    
    # TO PDF
    page_file_path_format = join(output_directory, "NSF_result.pdf")
    with gv.Viewer(test_files.sample_nsf) as viewer:
